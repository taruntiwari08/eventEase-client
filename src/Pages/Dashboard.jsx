...
import { getEventAnalytics, deleteEvent, updateEvent, getMyEvents } from "../Services/EventServices/eventapi";
...
function Dashboard() {
  ...
  return (
    <div className=" min-h-screen bg-gradient-to-r from-black via-blue-800 to-black text-white py-6 px-36">
      <ToastContainer position="top-right" autoClose={5000} />
      <h2 className="text-3xl font-bold mb-6 flex items-center gap-2">
        <IconChartBar size={28} className="text-orange-400" />
        Organizer Dashboard
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-indigo-800 rounded-xl p-4 shadow-lg flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold">Seats Sold</h3>
            <p className="text-2xl">
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-3 text-center mb-4">
                <div>
                  <p className="font-bold">{a.totalBookings || 0}</p>
                  <span className="text-xs text-gray-400">Bookings</span>
                </div>
                <div>
                  <p className="font-bold">{a.totalSeats || 0}</p>
                  <span className="text-xs text-gray-400">Seats</span>
                </div>
                <div>
                  <p className="font-bold">₹{a.totalRevenue || 0}</p>
              ...
              <div className="flex justify-end gap-3 mt-4">
                <button
                  ...
                  onClick={() => { setSelectedEvent(event); setOpen(true); }}
                >
                  <Edit size={18} /> Update Event Info
                </button>
                <button
                  className="bg-red-600 hover:bg-red-700 px-3 py-2 rounded-lg flex items-center gap-1"
                ...
                {open && selectedEvent && (
                  <UpdateEventModal
                    event={selectedEvent}
                    open={open}
                    onClose={() => { setOpen(false); setSelectedEvent(null); }}
